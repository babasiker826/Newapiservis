...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .about-section h2{color:var(--accent-1);margin-bottom:12px;font-size:20px;cursor:pointer;transition:color 0.2s ease;display:inline-flex;align-items:center;gap:8px}
        .about-section h2:hover{color:var(--accent-2)}
        .about-content{display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:16px}
        .about-card{background:rgba(255,255,255,0.03);padding:16px;border-radius:10px;border:1px solid rgba(255,255,255,0.05);cursor:pointer;transition:all 0.2s ease}
        .about-card:hover{background:rgba(255,255,255,0.05);transform:translateY(-2px);border-color:rgba(255,255,255,0.1)}
        ...
        .footer-text{cursor:pointer;transition:color 0.2s ease}
        .footer-text:hover{color:var(--accent-1)}
        ...
        @media (max-width: 768px) {
            ...
            .clickable-title{padding:6px 10px;font-size:16px}
        }
        ...
        @media (max-width: 480px) {
            ...
            .clickable-title{padding:4px 8px;font-size:14px}
        }

        /* small utility */
        .menu-dot{background:var(--glass-2);padding:8px;border-radius:8px;border:1px solid var(--card-border);cursor:pointer;display:grid;place-items:center;width:36px;height:36px;transition:all 0.2s ease}
        .menu-dot:hover{background:rgba(255,255,255,0.1)}

        /* modal */
        .modal{position:fixed;right:12px;top:60px;background:rgba(6,8,20,0.95);padding:10px;border-radius:10px;border:1px solid rgba(255,255,255,0.04);display:none;z-index:40;min-width:160px;backdrop-filter:blur(10px)}
        .modal.show{display:block}
        .modal a{display:block;padding:8px 10px;border-radius:6px;color:var(--muted);text-decoration:none;font-size:13px;transition:all 0.2s ease}
        .modal a:hover{background:rgba(255,255,255,0.02);color:#fff}

        /* toast */
        .toast{position:fixed;right:12px;bottom:12px;background:#0b1220;padding:8px 12px;border-radius:8px;border:1px solid var(--card-border);display:none;z-index:50;font-size:13px;backdrop-filter:blur(10px)}
    ...
    <div class="wrapper">
        <header>
            <div class="header-top">
                <div class="brand">
                    <div style="display:flex;flex-direction:column">
                        <h1 class="clickable-title" id="brandTitle">Nabi System</h1>
                        <p class="clickable-title" id="brandSubtitle">API Service • Mobile Uyumlu</p>
                    </div>
                </div>
            ...
            <div class="stats" aria-hidden="true">
                <div class="stat clickable-title" id="totalStat"><div class="num" id="totalApis">62</div><div class="label">Toplam API</div></div>
                <div class="stat clickable-title" id="activeStat"><div class="num" id="activeApis">62</div><div class="label">Aktif API</div></div>
            </div>
        </header>
        ...
        <main>
            <section>
                <div class="notice" id="warningNotice" role="status">
            ...
            <h2 class="section-title clickable-title" id="checkerTitle">🔐 Checker API'leri</h2>
            <div class="api-grid" id="checkerGrid"></div>

            <h2 class="section-title clickable-title" id="ratoTitle">🐀 Rato API'leri</h2>
            <div class="api-grid" id="ratoGrid"></div>

            <h2 class="section-title clickable-title" id="sosyalTitle">📱 Sosyal Medya API'leri</h2>
            <div class="api-grid" id="sosyalGrid"></div>

            <h2 class="section-title clickable-title" id="aiTitle">🤖 AI API'leri</h2>
            <div class="api-grid" id="aiGrid"></div>

            <h2 class="section-title clickable-title" id="tcTitle">🆔 TC Sorgulama API'leri</h2>
            <div class="api-grid" id="tcGrid"></div>

            <h2 class="section-title clickable-title" id="adsoyadTitle">👤 Ad Soyad Sorgulama</h2>
            <div class="api-grid" id="adsoyadGrid"></div>

            <h2 class="section-title clickable-title" id="isvergiTitle">💼 İş ve Vergi Sorgulama</h2>
            <div class="api-grid" id="isvergiGrid"></div>

            <h2 class="section-title clickable-title" id="tcgsmTitle">📱 TC-GSM Sorgulama</h2>
            <div class="api-grid" id="tcgsmGrid"></div>

            <h2 class="section-title clickable-title" id="adresTitle">🏠 Adres ve Konum Sorgulama</h2>
            <div class="api-grid" id="adresGrid"></div>

            <h2 class="section-title clickable-title" id="aileTitle">👨‍👩‍👧‍👦 Eş ve Aile Sorgulama</h2>
            <div class="api-grid" id="aileGrid"></div>

            <h2 class="section-title clickable-title" id="egitimTitle">🎓 Eğitim Sorgulama</h2>
            <div class="api-grid" id="egitimGrid"></div>

            <h2 class="section-title clickable-title" id="networkTitle">🌐 Network Sorgulama</h2>
            <div class="api-grid" id="networkGrid"></div>

            <h2 class="section-title clickable-title" id="leakTitle">🔓 Leak ve Telegram</h2>
            <div class="api-grid" id="leakGrid"></div>

            <h2 class="section-title clickable-title" id="ibanTitle">💰 IBAN API'leri</h2>
            <div class="api-grid" id="ibanGrid"></div>

            <h2 class="section-title clickable-title" id="sifreTitle">🔒 Şifre Encrypt</h2>
            <div class="api-grid" id="sifreGrid"></div>

            <h2 class="section-title clickable-title" id="payTitle">💳 Ödeme API'leri</h2>
            <div class="api-grid" id="payGrid"></div>
        </main>

        <footer>
            <div class="footer-text" id="footerMain">NABI SYSTEM SUNAR — v2 • 62+ API • Mobile Uyumlu</div>
            <div style="margin-top:6px;color:var(--muted);font-size:11px" class="footer-text" id="footerSub">© 2025 Nabi System • Telegram: @sukazatkinis</div>
        </footer>
    </div>
    ...
    <script>
        ...
        function makeCard(item){
            ...
            card.innerHTML = `
                ...
                <div class="api-url" data-url="${item.url}" tabindex="0">${item.url}</div>
        ...
        // Tıklanabilir başlıklar için event listener'lar
        document.getElementById('brandTitle').addEventListener('click', () => {
            showToast('Nabi System - Hoş geldiniz!');
            addClickEffect(document.getElementById('brandTitle'));
        });

        document.getElementById('brandSubtitle').addEventListener('click', () => {
            showToast('Mobile uyumlu API servisi');
            addClickEffect(document.getElementById('brandSubtitle'));
        });

        document.getElementById('totalStat').addEventListener('click', () => {
            showToast('Toplam API sayısı: ' + document.getElementById('totalApis').innerText);
            addClickEffect(document.getElementById('totalStat'));
        });

        document.getElementById('activeStat').addEventListener('click', () => {
            showToast('Aktif API sayısı: ' + document.getElementById('activeApis').innerText);
            addClickEffect(document.getElementById('activeStat'));
        });

        document.getElementById('warningNotice').addEventListener('click', () => {
            showToast('Gizlilik ve yasalara dikkat ediniz!');
            addClickEffect(document.getElementById('warningNotice'));
        });

        document.getElementById('aboutTitle').addEventListener('click', () => {
            showToast('Nabi System hakkında bilgiler');
            addClickEffect(document.getElementById('aboutTitle'));
        });

        // About kartları
        document.getElementById('statsCard').addEventListener('click', () => {
            showToast('Servis istatistikleri görüntülendi');
            addClickEffect(document.getElementById('statsCard'));
        });

        document.getElementById('techCard').addEventListener('click', () => {
            showToast('Teknoloji stack bilgileri');
            addClickEffect(document.getElementById('techCard'));
        });

        document.getElementById('usageCard').addEventListener('click', () => {
            showToast('Kullanım alanları bilgisi');
            addClickEffect(document.getElementById('usageCard'));
        });

        document.getElementById('contactCard').addEventListener('click', () => {
            showToast('İletişim bilgileri kopyalandı');
            navigator.clipboard.writeText('Telegram: @sukazatkinis');
            addClickEffect(document.getElementById('contactCard'));
        });

        // Section başlıkları
        const sectionTitles = ['checkerTitle', 'ratoTitle', 'sosyalTitle', 'aiTitle', 'tcTitle', 
                              'adsoyadTitle', 'isvergiTitle', 'tcgsmTitle', 'adresTitle', 'aileTitle',
                              'egitimTitle', 'networkTitle', 'leakTitle', 'ibanTitle', 'sifreTitle', 'payTitle'];
        
        sectionTitles.forEach(titleId => {
            document.getElementById(titleId).addEventListener('click', () => {
                const titleText = document.getElementById(titleId).textContent.replace(/[🔐🐀📱🤖🆔👤💼📱🏠👨‍👩‍👧‍👦🎓🌐🔓💰🔒💳]/g, '').trim();
                showToast(titleText + ' kategorisi');
                addClickEffect(document.getElementById(titleId));
            });
        });

        // Footer tıklama
        document.getElementById('footerMain').addEventListener('click', () => {
            showToast('Nabi System v2 - 62+ API');
            addClickEffect(document.getElementById('footerMain'));
        });

        document.getElementById('footerSub').addEventListener('click', () => {
            navigator.clipboard.writeText('@sukazatkinis');
            showToast('Telegram kopyalandı: @sukazatkinis');
            addClickEffect(document.getElementById('footerSub'));
        });
        ...
        document.getElementById('toggleBg').addEventListener('click', ()=>{
            ...
            addClickEffect(document.getElementById('toggleBg'));
        });
        ...
        document.getElementById('downloadJson').addEventListener('click', (e)=>{
            ...
            addClickEffect(document.getElementById('downloadJson'));
        });

        document.getElementById('about').addEventListener('click', (e)=>{ 
            e.preventDefault(); 
            alert('Nabi System — API servisi v2\\nMobile uyumlu, kopyala / aç / indir özellikleri eklendi.\\nToplam 62+ aktif API!'); 
            addClickEffect(document.getElementById('about'));
        });

        document.getElementById('openEditor').addEventListener('click', (e)=>{ 
            e.preventDefault(); 
            showToast('Bu buton örnektir — manuel düzenleyin.'); 
            addClickEffect(document.getElementById('openEditor'));
        });
